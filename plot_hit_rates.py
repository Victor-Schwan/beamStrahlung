...
scenarios = {
  "182GeV_nzco_10urad": {"label": "SR Core (Scaled)", "bx_rate": 50000000.0, 'c': 'b',},
  "FCC091": {"label": "FCC091", "bx_rate": 50000000.0, 'c': 'g',},
  "FCC240": {"label": "FCC240", "bx_rate": 50000000.0, 'c': 'orange',},
  "ILC250": {"label": "ILC250", "bx_rate": 6600.0, 'c': 'purple',},
  "182GeV_halo": {"label": "SR Halo (Scaled)", "bx_rate": 50000000.0, 'c': 'brown',},
}
...
def plot_endcap(rows, axes, det_mod):
  ...
  for scenario in scenarios.keys():
    ...
    if len(hit_rates) == len(z_positions):
      axes.plot(z_positions, hit_rates*dict["bx_rate"], marker='.', label=dict['label'], c=dict['c'])
...
def plot_barrel(rows, axes, det_mod):
  ...
  for scenario in scenarios.keys():
    ...
    if len(hit_rates) == len(radii):
      axes.plot(radii, hit_rates*dict["bx_rate"], marker='.', label=dict['label'], c=dict['c'])
