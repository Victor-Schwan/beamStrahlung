...
def get_areas():
    ...
    fcc_barrel_area = calculate_barrel_area(
        constants["VertexBarrel_r"], constants["VertexBarrel_zmax"], sub_det_cols_fcc
    )
    fcc_endcap_area = (
        pi
        * constants["VertexEndcap_rmax"][0] ** 2
        * len(constants["VertexEndcap_z"])
        * 2
    )

    ilc_barrel_area = calculate_barrel_area(
        ILC_RADII, ILC_HALF_LENGTHS, sub_det_cols_ilc
    )
