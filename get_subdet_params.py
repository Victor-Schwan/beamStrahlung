...
small_TPC_xml = (
    "../k4geo/FCCee/ILD_FCCee/compact/ILD_FCCee_v02/top_defs_ILD_FCCee_v02.xml"
)
...
def split_double_layers(parameters):
    for det_mod, det_params in parameters.items():
        if SUB_DET_COLS[det_mod]["vb"].only_double_layers:
            parameters[det_mod]["Vertex"]["vb"]["r"] = [
                radius for r in det_params["Vertex"]["vb"]["r"] for radius in (r, r + 2)
            ]
            parameters[det_mod]["Vertex"]["vb"]["z"] = [
                half_length
                for z in det_params["Vertex"]["vb"]["z"]
                for half_length in (z, z)
            ]
        if det_mod.split("_")[1] != "FCCee":
            continue
        if SUB_DET_COLS[det_mod]["ve"].only_double_layers:
            parameters[det_mod]["Vertex"]["ve"]["r"] = [
                radius for r in det_params["Vertex"]["ve"]["r"] for radius in (r, r)
            ]
            parameters[det_mod]["Vertex"]["ve"]["z"] = [
                pos for z in det_params["Vertex"]["ve"]["z"] for pos in (z, z + 2)
            ]
        ...
        new_params[det_mod] = {
            **det_params,
            "Vertex": {
                **det_params["Vertex"],
                "vb": {
                    **det_params["Vertex"]["vb"],
                    "a": vb_areas,
                    "n_pixels": vb_pixels,
                },
                "ve": {
                    **det_params["Vertex"]["ve"],
                    "a": ve_areas,
                    "n_pixels": ve_pixels,
                },
            },
        ...
        "ILD_FCCee_v01": {
            "Vertex": {
                "vb": {
                    ...
                    "z": fcc_vertex_params["VertexBarrel_zmax"]
                    * len(fcc_vertex_params["VertexBarrel_r"]),
                    "a": [],
                    "n_pixels": [],
                },
                "ve": {
                    "r": fcc_vertex_params["VertexEndcap_rmax"]
                    * len(fcc_vertex_params["VertexEndcap_z"]),
