...
def split_all_hepevt_files_in_dir(lines_per_file=5000, file_pattern="*.hepevt"):
    hepevt_files = list(SR_raw_input_dir.glob(file_pattern))
    if not hepevt_files:
        print(
            f"No HEPEVT files matching '{file_pattern}' found in '{SR_raw_input_dir}'"
        )
        return

    for hepevt_file in hepevt_files:
        # Create a unique output directory for each original HEPEVT file
        sub_output_dir = SR_split_input_dir / hepevt_file.stem
        split_hepevt_file(
